...
int main() {
    ...
    cout << "Enter the initial approximation to the eigenvector" << endl;
    ...
    cout << fixed;
    cout << "Itr no." << setw(11) << "Eigenvalue" << setw(19) << "EigenVector" << endl;

    findmax(&e, x); // initial max of eigenvector

    for (itr = 1; itr <= maxitr; itr++) {
        // multiply matrix a with vector x
        for (i = 0; i < N; i++) {
            s = 0;
            for (k = 0; k < N; k++) s += a[i][k] * x[k];
            r[i] = s;
        }

        findmax(&t, r); // normalize vector r
        for (i = 0; i < N; i++) r[i] /= t;
        ...
        cout << setw(4) << itr << setw(12) << setprecision(4) << e;
        for (i = 0; i < N; i++) cout << setw(9) << setprecision(3) << x[i];
        cout << endl;

        if ((errv <= aerr) && (maxe <= aerr)) {
            cout << "Converges in " << itr << " iterations" << endl;
            cout << "Largest eigenvalue = " << setw(6) << setprecision(2) << e << endl;
            cout << "Eigenvector:-" << endl;
            for (i = 0; i < N; i++)
                cout << "x[" << setw(3) << i + 1 << "] = " << setw(6) << setprecision(2) << x[i] << endl;
            cout << endl;
            return 0;
        }
    }
